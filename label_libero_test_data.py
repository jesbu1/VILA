...
CUDA_VISIBLE_DEVICES=0 python label_libero_test_data.py \
...
def generate_paths_masks(args: Args) -> None:
    ...
    for raw_dataset_name in RAW_DATASET_NAMES:
        # Load dataset
        print(
            f"-------------------------Processing {raw_dataset_name}-------------------------"
        )
        # open the directory containing the h5 files using raw_dataset_name
        libero_h5_list = [
            file
            for file in os.listdir(Path(args.data_dir) / raw_dataset_name)
            if file.endswith(".hdf5")
        ]
        libero_h5_list.sort()
        for libero_h5_file in libero_h5_list:
            h5_path = output_path / f"{raw_dataset_name}_{libero_h5_file.split('.')[0]}_paths_masks.h5"
            print(f"Saving to {h5_path}")
            with h5py.File(h5_path, "a") as f:
                # Process each episode. Resume by going to the second from last episode.
                already_saved_episodes = sorted(
                    [int(k.split("_")[-1]) for k in f.keys()]
                )
                ...
                with h5py.File(
                    Path(args.data_dir) / raw_dataset_name / libero_h5_file,
                    "r",
                    swmr=True,
                ) as libero_trajectory_f:
                    for episode_idx, demo_name in tqdm.tqdm(
                        enumerate(libero_trajectory_f["data"]),
                        ...
                        # Delete the last episode if it exists, as it might be incomplete
                            ...
                            logging.info(
                                f"Deleting episode {episode_idx} as it is incomplete"
                            )
                            try:
                        ...
                        num_steps = len(
                            libero_trajectory_f["data"][demo_name]["obs"]["ee_pos"]
                        )

                        # Compute the main language instruction
                        if "problem_info" in libero_trajectory_f["data"].attrs:
                            task_description = json.loads(
                                libero_trajectory_f["data"].attrs["problem_info"]
                            )["language_instruction"]
                        else:
                            # openvla's language instruction extraction method as it doesn't have problem_info
                            raw_file_string = os.path.basename(libero_h5_file).split("/")[
                                -1
                            ]
                            ...
                            for w in words:
                                if "SCENE" in w:
                                    task_description = ""
                                    continue
                                task_description = task_description + w + " "
                        ...
                        for i in range(num_steps):
                            ...
                            agentview_img = libero_trajectory_f["data"][demo_name]["obs"][
                                "agentview_rgb"
                            ][i][::-1]  # flip the image as it comes from LIBERO reversed
                            step_images.append(agentview_img)
                            step_tasks.append(task_description)
                            step_timesteps.append(i)
                            step_cameras.append("image")

                            if not step_images:
                                logging.warning(
                                    f"No valid images found in step {i} of episode {episode_idx}"
                                )
                                continue
                        ...
                        if not episode_images:
                            logging.warning(
                                f"No valid images found in episode {episode_idx}"
                            )
                            continue
